...
<header class="group my-6 flex w-full items-center px-6">
  ...
  >
    <nav
      class={`flex scrollbar-hidden grow items-center gap-x-1 overflow-x-auto transition-all ease-out sm:max-w-500 ${showNav ? 'max-sm:max-w-500 max-sm:delay-100 max-sm:duration-1000' : 'max-sm:max-w-0'}`}
    >
      <ul class="contents">
        <NavLink href={resolve('/')}>Home</NavLink>
        <NavLink href={resolve('/books')}>Books</NavLink>
        <NavLink href={resolve('/about')}>About</NavLink>
        <NavLink href={resolve('/settings')} onclick={props.onSettingsClick}
          ><SettingsIcon class="text-center align-baseline" /></NavLink
        >
      </ul>
    </nav>
